...
EvalScore eval_minor_coordination(const Board& board, Color c, [[maybe_unused]] EvalContext& ctx) {
    EvalScore score;
...
static void do_full_evaluation(const Board& board, EvalScore& score, [[maybe_unused]] int phase) {
    EvalContext ctx;
...
static int finalize_score(const EvalScore& score, int phase, Color stm) {
    int mg = score.mg;
    int eg = score.eg;
    int finalScore = (mg * phase + eg * (TotalPhase - phase)) / TotalPhase;
    constexpr int TEMPO = 12;
    return (stm == WHITE ? finalScore : -finalScore) + TEMPO;
}
...
int evaluate(const Board& board, int alpha, int beta) {
    ...
    int cachedScore;
    if (evalCache.probe(posKey, cachedScore, alpha, beta)) {
        return cachedScore;
    }
...

    {
        ...
        constexpr int LAZY_MARGIN = 1500;
        if (lazyScore >= beta + LAZY_MARGIN || lazyScore <= alpha - LAZY_MARGIN) {
            return lazyScore;
        }
    }

    do_full_evaluation(board, score, phase);

    int finalScore = finalize_score(score, phase, board.side_to_move());

    evalCache.store(posKey, finalScore, alpha, beta);
...
int evaluate_no_cache(const Board& board) {
    ...
    return finalize_score(score, phase, board.side_to_move());
}
