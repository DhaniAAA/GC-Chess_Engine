...
bool MoveGen::is_legal(const Board& board, Move m) {
    ...
    // Guard: menangkap raja musuh tidak pernah legal.
    // Move ke kotak raja seharusnya tidak pernah terjadi di posisi valid,
    // tapi guard ini mencegah board corrupt jika ada state yang tidak valid.
    if (type_of(board.piece_on(to)) == KING) {
        return false;
    }
