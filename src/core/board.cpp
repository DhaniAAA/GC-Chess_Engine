...
bool Board::is_attacked_by(Color c, Square s) const {
    Bitboard occ = pieces();
    // Short-circuit per piece type — berhenti dini saat attacker ditemukan
    if (pawn_attacks_bb(~c, s)   & pieces(c, PAWN))          return true;
    if (knight_attacks_bb(s)     & pieces(c, KNIGHT))        return true;
    if (king_attacks_bb(s)       & pieces(c, KING))          return true;
    if (bishop_attacks_bb(s,occ) & pieces(c, BISHOP, QUEEN)) return true;
    if (rook_attacks_bb(s, occ)  & pieces(c, ROOK, QUEEN))   return true;
    return false;
}
