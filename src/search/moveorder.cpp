...
SEE::SEECacheEntry SEE::cache[SEECACHE_SIZE];

void SEE::clear_cache() {
    for (int i = 0; i < SEECACHE_SIZE; ++i) {
        cache[i].key = 0;
        cache[i].move16 = 0;
        cache[i].value = 0;
    }
...
int SEE::evaluate(const Board& board, Move m) {
    ...
    Key posKey = board.key();
    U16 m16 = m.raw();
    SEECacheEntry& entry = cache[(posKey ^ m16) & SEECACHE_MASK];

    if (entry.key == posKey && entry.move16 == m16) {
        return entry.value;
    }
    ...
    entry.key = posKey;
    entry.move16 = m16;
    entry.value = static_cast<int16_t>(result);
...
MovePicker::MovePicker(const Board& b, const Move* tm, int count, int p,
    ...
    : board(b), history(ht), killers(&kt), counterMoves(&cm),
      contHist1ply(contHist1), contHist2ply(contHist2), contHist3ply(contHist3),
      captureHist(ch),
...
void MovePicker::score_captures() {
    ...
    for (int idx = 0; idx < moveCount; ++idx) {
        ...
        sm.seeValue = static_cast<int16_t>(see_value);
...
void MovePicker::score_quiets() {
    ...
    Bitboard checkSquares = board.check_squares(PAWN) | board.check_squares(KNIGHT) |
                            board.check_squares(BISHOP) | board.check_squares(ROOK) | board.check_squares(QUEEN);
    ...
    for (int idx = 0; idx < moveCount; ++idx) {
        ...
        if (checkSquares & square_bb(to)) {
            tacticalBonus += 8000;
        }

        if ((pinned & square_bb(m.from())) && !aligned(m.from(), to, enemyKing)) {
            tacticalBonus += 4000;
        }
        ...
        if (m == killer1) {
            ...
            sm.score = 2 * histScore + cont1 + cont2 + cont3 + tacticalBonus;
        }
